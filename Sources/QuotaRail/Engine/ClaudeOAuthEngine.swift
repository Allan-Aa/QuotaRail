...
/// wrote to disk when you ran `claude auth login`; nothing is sent anywhere except
...
enum ClaudeOAuthEngine {
    ...
    enum Failure: Error {
        ...
        var userMessage: String {
            switch self {
            case .credentialsUnavailable:
                return "未找到 Claude Code 登录凭据。请先运行 claude auth login。"
            case .credentialsExpired:
                return "Claude 登录已过期。请运行 claude auth login。"
            case .requestDenied:
                return "Claude 用量接口拒绝了访问。请稍后重试或检查账号权限。"
            case .requestFailed:
    ...
    private static func loadCredentials() -> CredentialLoad {
        let fromFile = loadCredentialsFromFile()
        if case .success = fromFile { return fromFile }

        let fromKeychain = loadCredentialsFromKeychain()
        if case .success = fromKeychain { return fromKeychain }

        switch (fromFile, fromKeychain) {
        case (.expired, _), (_, .expired): return .expired
        default: return .unavailable
        }
    }

    private static func loadCredentialsFromFile() -> CredentialLoad {
        guard let home = ProcessInfo.processInfo.environment["HOME"] else { return .unavailable }
        let path = home + "/.claude/.credentials.json"
        guard let data = FileManager.default.contents(atPath: path) else { return .unavailable }
    ...
    private static func loadCredentialsFromKeychain() -> CredentialLoad {
        let process = Process()
        let output = Pipe()
    ...
    private static func parseCredentials(_ data: Data) -> CredentialLoad {
    ...
    static func computeSnapshot() -> Result<Snapshot, Failure> {
        ...
        case .success(let loaded): credentials = loaded
        case .unavailable: return .failure(.credentialsUnavailable)
        case .expired: return .failure(.credentialsExpired)
        }
    ...
    private static func fetchUsageData(accessToken: String) -> Result<Data, Failure> {
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, _ in
            ...
            if http.statusCode == 401 {
                result = .failure(.credentialsExpired)
                return
            }
            if http.statusCode == 403 {
                result = .failure(.requestDenied)
                return
            }
    ...
    private static func accessTokenIsExpired(_ accessToken: String, now: Date = Date()) -> Bool {
